...
if best_error > 0.05
    FS_error = zeros(1, size(year3, 2)-1);
    for j = 1:size(year3,2)-1
        error = 1;
              
        % l = int8(ismember(F, ii));
        if ismember(F_, j) == logical(0);
            
            ind = find(ismember(F_, 0)); 
            F = F_; F(ind) = [];
            F = [F j];
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            X1 = year3(:, F); Y1 = year3(:,end); % Can change the year
            k = 10; SVMerror = zeros(1, k);
            ...
            for i = 1:k
                ...
                FS_SVM_error(i) = test_errorSVM;
            end
            F(end) = [];
            error = min(FS_SVM_error);
            
        end
        FS_error(j) = error;
    end
    [best_error I] = min(FS_error);
    F_ = [F I];
end